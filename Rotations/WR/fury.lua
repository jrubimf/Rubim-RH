...
local function AoE()
    ...
    if CDsON() and S.Bladestorm:IsCastable() and Player:BuffRemainsP(S.Enrage) > 2 and Cache.EnemiesCount[8] > 1 then
...
local function CDs()
    ...
    if S.Bloodthirst:IsCastable() and Target:Exists() and Target:HealthPercentage() < 20 and Player:BuffRemainsP(S.Enrage) < Player:GCD() then
        return S.Bloodthirst:ID()
    end
...
local function execute()
    ...
    if S.FuriousSlash:IsCastable() and T192 and Target:TimeToDie() >= 10 then
        return S.FuriousSlash:ID()
    end
...
local function single_target()
    ...
    if S.FuriousSlash:IsCastable() and T192 and not S.InnerRage:IsAvailable() then
        return S.FuriousSlash:ID()
    end
...
function WarriorFury()
    ...
    if Player:Buff(S.EnragedRegeneration) and S.Bloodthirst:IsCastable() and Player:HealthPercentage() <= 90 then
        return S.Bloodthirst:ID()
    end
    ...
    if S.DragonRoar:IsCastable() and Cache.EnemiesCount[10] >= 1
