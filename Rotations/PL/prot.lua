...
function PProtRotation()
    ...
    if Target:Exists() and Player:CanAttack(Target) and not Target:IsDeadOrGhost() then
      -- CDs
        ...
        if S.ShieldoftheRighteous:IsCastable("Melee") and Player:BuffRefreshable(S.ShieldoftheRighteousBuff, 4) and (Player:ActiveMitigationNeeded() or Player:HealthPercentage() <= 90 or (not S.AvengersShield:CooldownUp() and S.ShieldoftheRighteous:ChargesFractional() >= 2.65)) then
        ...
        -- Eye of Tyr (HP)
        if S.EyeofTyr:IsCastable(8, true) and Player:HealthPercentage() <= 85 then
          return S.EyeofTyr:ID()
        end
      -- Defensives
