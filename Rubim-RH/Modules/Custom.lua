...
local constantLoop = CreateFrame("Frame");
constantLoop:SetScript("OnUpdate", function(self, sinceLastUpdate)
	constantLoop:onUpdate(sinceLastUpdate);
	end)

function constantLoop:onUpdate(sinceLastUpdate)
	...
	if (self.sinceLastUpdate >= 0.2) then
		...
		if IsMouseButtonDown()
			...
			if GetActionInfo(EAB) 
				...
				if QueuedSpell ~= spellid then
					QueuedSpell = RubimRH.Spell[251].DeathStrike:Queue()
					print("RubimRH - Queued: " .. GetSpellInfo(spellid))
				end	
				--print("overhere")
				--ActionButton_ShowOverlayGlow(GetMouseFocus():GetName())
			end
		end
