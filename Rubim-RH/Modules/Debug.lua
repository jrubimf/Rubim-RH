...
function debugGuiShow()
    ...
    green:SetCallback("OnEnterPressed", function(widget, event, text)
        RubimRH.DebugFrame.texture:SetColorTexture(0, tonumber(text), 0, 1.0)
    end)
    debugGUI:AddChild(green)

    local red = AceGUI:Create("EditBox")
    --sizeZ:GetText(select(1, RubimRH.DebugFrame:GetSize()))
    red:SetText(select(1, RubimRH.DebugFrame:GetSize()))
    red:SetLabel("Green:")
    red:SetWidth(300)
    red:SetCallback("OnEnterPressed", function(widget, event, text)
        RubimRH.DebugFrame.texture:SetColorTexture(tonumber(text), 0, 0, 1.0)
    end)
    debugGUI:AddChild(red)

    local blue = AceGUI:Create("EditBox")
    --sizeZ:GetText(select(1, RubimRH.DebugFrame:GetSize()))
    blue:SetText(select(1, RubimRH.DebugFrame:GetSize()))
    blue:SetLabel("Blue:")
    blue:SetWidth(300)
    blue:SetCallback("OnEnterPressed", function(widget, event, text)
        RubimRH.DebugFrame.texture:SetColorTexture(0, 0, tonumber(text), 1.0)
    end)
    debugGUI:AddChild(blue)

    local yellow = AceGUI:Create("EditBox")
    --sizeZ:GetText(select(1, RubimRH.DebugFrame:GetSize()))
    yellow:SetText(select(1, RubimRH.DebugFrame:GetSize()))
    yellow:SetLabel("Blue:")
    yellow:SetWidth(300)
    yellow:SetCallback("OnEnterPressed", function(widget, event, text)
        RubimRH.DebugFrame.texture:SetColorTexture(tonumber(text), tonumber(text), 0, 1.0)
    end)
    debugGUI:AddChild(yellow)
