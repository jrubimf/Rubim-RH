-- Listener
Listener, listeners = {}, {}
local frame = CreateFrame('Frame', 'Ayni_Events')
...
getAura = function(guid, spellIdOrName, filter)
    ...
    if not aura then
        return
    elseif filters["PLAYER"] and aura.unitCaster ~= "player" then
        return
    end
...
getAurasForUnit = function(unitId, guid)
    ...
    for i = 1, 40 do
        if not addBuff(unitId, guid, i) then
            break
        end
    end
    for i = 1, 40 do
        if not addDebuff(unitId, guid, i) then
            break
        end
    end
end
