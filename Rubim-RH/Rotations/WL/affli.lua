...
local function TimeToShard()
  local ActiveAgony = NbAffected(S.Agony)
...
local function APL()
  ...
  Precombat = function()
    ...
    if S.SeedofCorruption:IsCastableP() and RubimRH.AoEON() and Player:SoulShardsP() >= 1 and (not Player:IsMoving()) and not Player:ShouldStopCasting() and Player:DebuffDownP(S.SeedofCorruptionDebuff) and (active_enemies() >= 3) then
  ...
  DbRefresh = function()
    ...
    if S.SiphonLife:IsCastableP() and ((Target:DebuffRemainsP(S.SiphonLifeDebuff) / S.SiphonLife:BaseDuration()) <= (Target:DebuffRemainsP(S.AgonyDebuff) / S.Agony:BaseDuration()) and (Target:DebuffRemainsP(S.SiphonLifeDebuff) / S.SiphonLife:BaseDuration()) <= (Target:DebuffRemainsP(S.CorruptionDebuff) / S.Corruption:BaseDuration()) and Target:DebuffRemainsP(S.SiphonLifeDebuff) < S.SiphonLife:BaseDuration() * 1.3) then
    ...
    if S.Agony:IsCastableP() and ((Target:DebuffRemainsP(S.AgonyDebuff) / S.Agony:BaseDuration()) <= (Target:DebuffRemainsP(S.CorruptionDebuff) / S.Corruption:BaseDuration()) and (Target:DebuffRemainsP(S.AgonyDebuff) / S.Agony:BaseDuration()) <= (Target:DebuffRemainsP(S.SiphonLifeDebuff) / S.SiphonLife:BaseDuration()) and Target:DebuffRemainsP(S.AgonyDebuff) < S.Agony:BaseDuration() * 1.3) then
      return S.Agony:Cast()
    end
    ...
    if S.Corruption:IsCastableP() and ((Target:DebuffRemainsP(S.CorruptionDebuff) / S.Corruption:BaseDuration()) <= (Target:DebuffRemainsP(S.AgonyDebuff) / S.Agony:BaseDuration()) and (Target:DebuffRemainsP(S.CorruptionDebuff) / S.Corruption:BaseDuration()) <= (Target:DebuffRemainsP(S.SiphonLifeDebuff) / S.SiphonLife:BaseDuration()) and Target:DebuffRemainsP(S.CorruptionDebuff) < S.Corruption:BaseDuration() * 1.3) then
  ...
  Dots = function()
    ...
    if S.Corruption:IsCastableP() and (not S.AbsoluteCorruption:IsAvailable()) and not Player:PrevGCDP(1, S.Corruption) and not Target:Debuff(S.CorruptionDebuff) and HL.CombatTime() <= 5 then
        return S.Corruption:Cast()
    end	
    ...
    if S.SiphonLife:IsCastableP() and (Target:DebuffRemainsP(S.SiphonLifeDebuff) < 8 - num(S.CreepingDeath:IsAvailable())) and Target:TimeToDie() > 10 and Target:DebuffRefreshableCP(S.SiphonLifeDebuff) and (not Target:DebuffRemainsP(S.SiphonLifeDebuff) or S.SummonDarkglare:CooldownRemainsP() > Player:SoulShardsP() * S.UnstableAffliction:ExecuteTime()) then
      return S.SiphonLife:Cast()
    end
	-- siphon_life,target=1,!siphonlife.debuff
    if S.SiphonLife:IsCastableP() and not Target:Debuff(S.SiphonLifeDebuff) then
        return S.SiphonLife:Cast()
    end	
 ...
 --   if S.Corruption:IsCastableP() and Cache.EnemiesCount[40] < 3 + num(S.WritheInAgony:IsAvailable()) and (Target:DebuffRemainsP(S.CorruptionDebuff) <= Player:GCD()) or (S.SummonDarkglare:CooldownRemainsP() > 10 and Target:DebuffRefreshableCP(S.CorruptionDebuff)) and Target:TimeToDie() > 10 then
 --     return S.Corruption:Cast()
 --   end
  ...
  Fillers = function()
    ...
    if S.UnstableAffliction:IsReadyP() and (not Player:IsMoving()) and not Player:ShouldStopCasting() and (S.Deathbolt:CooldownRemainsP() <= Player:GCD() * 2 and S.SummonDarkglare:CooldownRemainsP() > 20) then
    ...
    if  RubimRH.CDsON() and ((Target:DebuffRemainsP(S.AgonyDebuff) < S.Agony:BaseDuration() * 0.75 or Target:DebuffRemainsP(S.CorruptionDebuff) < S.Corruption:BaseDuration() * 0.75 or Target:DebuffRemainsP(S.SiphonLifeDebuff) < S.SiphonLife:BaseDuration() * 0.75) and S.Deathbolt:CooldownRemainsP() <= Player:GCD() * 4 and S.SummonDarkglare:CooldownRemainsP() > 20) then
		local ShouldReturn = DbRefresh(); 
			if ShouldReturn then return ShouldReturn; 
		end
    end	
	-- call_action_list,name=db_refresh,if=spell_targets.seed_of_corruption_aoe=1+raid_event.invulnerable.up&(dot.agony.remains<dot.agony.duration*0.75|dot.corruption.remains<dot.corruption.duration*0.75|dot.siphon_life.remains<dot.siphon_life.duration*0.75)&cooldown.deathbolt.remains<=action.agony.gcd*4&cooldown.summon_darkglare.remains>20
    if  (not RubimRH.CDsON()) and ((Target:DebuffRemainsP(S.AgonyDebuff) < S.Agony:BaseDuration() * 0.75 or Target:DebuffRemainsP(S.CorruptionDebuff) < S.Corruption:BaseDuration() * 0.75 or Target:DebuffRemainsP(S.SiphonLifeDebuff) < S.SiphonLife:BaseDuration() * 0.75) and S.Deathbolt:CooldownRemainsP() <= Player:GCD() * 4) then
		local ShouldReturn = DbRefresh(); 
			if ShouldReturn then return ShouldReturn; 
		end
    end
	...
	-- call_action_list,name=db_refresh,if=spell_targets.seed_of_corruption_aoe=1+raid_event.invulnerable.up&cooldown.summon_darkglare.remains<=soul_shard*action.agony.gcd+action.agony.gcd*3&(dot.agony.remains<dot.agony.duration*1|dot.corruption.remains<dot.corruption.duration*1|dot.siphon_life.remains<dot.siphon_life.duration*1)
    if  (not RubimRH.CDsON()) and (Target:DebuffRemainsP(S.AgonyDebuff) < S.Agony:BaseDuration() * 0.25 or Target:DebuffRemainsP(S.CorruptionDebuff) < S.Corruption:BaseDuration() * 0.25 or Target:DebuffRemainsP(S.SiphonLifeDebuff) < S.SiphonLife:BaseDuration() * 0.25) then
		local ShouldReturn = DbRefresh(); 
			if ShouldReturn then return ShouldReturn; 
		end
    end
    ...
    -- haunt
    if S.Haunt:IsCastableP() and (not Player:IsMoving()) and not Player:ShouldStopCasting() then
      return S.Haunt:Cast()
    end
    -- drain_soul,interrupt_global=1,chain=1,interrupt=1,cycle_targets=1,if=target.time_to_die<=gcd
    if S.DrainSoul:IsCastableP() and (not Player:IsMoving()) and not Player:ShouldStopCasting() and Target:TimeToDie() <= Player:GCD()  then
    ...
    if S.DrainSoul:IsCastableP() and Target:DebuffRemainsP(S.ShadowEmbraceDebuff) and S.ShadowEmbrace:IsAvailable() and bool(VarMaintainSe) and (not Target:DebuffRemainsP(S.ShadowEmbraceDebuff)) and (not Player:IsMoving()) and not Player:ShouldStopCasting() then
      return S.DrainSoul:Cast()
    end
    ...
    if S.DrainSoul:IsCastableP() and Target:DebuffRemainsP(S.ShadowEmbraceDebuff) and S.ShadowEmbrace:IsAvailable() and bool(VarMaintainSe) and (not Player:IsMoving()) and not Player:ShouldStopCasting() then
      return S.DrainSoul:Cast()
    end
    -- drain_soul,interrupt_global=1,chain=1,interrupt=1
    if S.DrainSoul:IsCastableP() and (not Player:IsMoving()) and not Player:ShouldStopCasting() then
      return S.DrainSoul:Cast()
    end
    ...
    if S.ShadowBolt:IsCastableP() and S.ShadowEmbrace:IsAvailable() and bool(VarMaintainSe) and (not Target:DebuffRemainsP(S.ShadowEmbraceDebuff)) and (not S.ShadowBolt:InFlight()) and (not Player:IsMoving()) and not Player:ShouldStopCasting() then
      return S.ShadowBolt:Cast()
    end
    -- shadow_bolt,target_if=min:debuff.shadow_embrace.remains,if=talent.shadow_embrace.enabled&variable.maintain_se
    if S.ShadowBolt:IsCastableP() and S.ShadowEmbrace:IsAvailable() and bool(VarMaintainSe) and (not Player:IsMoving()) and not Player:ShouldStopCasting() then
      return S.ShadowBolt:Cast()
    end
    -- shadow_bolt
    if S.ShadowBolt:IsCastableP() and (not Player:IsMoving()) and not Player:ShouldStopCasting() then
      return S.ShadowBolt:Cast()
    end
  end
  ...
  Spenders = function()
    ...
    if S.UnstableAffliction:IsReadyP() and (not Player:IsMoving()) and not Player:ShouldStopCasting() and (S.SummonDarkglare:CooldownRemainsP() <= Player:SoulShardsP() * S.UnstableAffliction:ExecuteTime() and (not S.Deathbolt:IsAvailable() or S.Deathbolt:CooldownRemainsP() <= Player:SoulShardsP() * S.UnstableAffliction:ExecuteTime())) then
      return S.UnstableAffliction:Cast()
    end
    ...
    if  RubimRH.CDsON() and ((S.SummonDarkglare:CooldownRemainsP() < time_to_shard * (6 - Player:SoulShardsP()) or S.SummonDarkglare:CooldownUpP()) and Target:TimeToDie() > S.SummonDarkglare:CooldownRemainsP()) then
		local ShouldReturn = Fillers(); 
			if ShouldReturn then return ShouldReturn; 
		end
    end
    -- seed_of_corruption,if=variable.use_seed
    if S.SeedofCorruption:IsCastableP() and RubimRH.AoEON() and active_enemies() >= 3 and Player:SoulShardsP() >= 1 and (not Player:IsMoving()) and not Player:ShouldStopCasting() and (bool(VarUseSeed)) then
      return S.SeedofCorruption:Cast()
    end
    ...
    if S.UnstableAffliction:IsReadyP() and (not Player:IsMoving()) and not Player:ShouldStopCasting() and not bool(VarUseSeed) and contagion <= S.UnstableAffliction:CastTime() + VarPadding then
      return S.UnstableAffliction:Cast()
    end
  ...
  if RubimRH.TargetIsValid() then
    ...
    if S.SummonDarkglare:IsCastableP() and RubimRH.CDsON() and (Target:DebuffP(S.AgonyDebuff) and (Target:DebuffP(S.CorruptionDebuff) or S.AbsoluteCorruption:IsAvailable()) and (ActiveUAs() == 5 or (Player:SoulShardsP() < 1 and ActiveUAs() >= 4)) and (not S.PhantomSingularity:IsAvailable() or bool(S.PhantomSingularity:CooldownRemainsP())) and (not S.Deathbolt:IsAvailable() or S.Deathbolt:CooldownRemainsP() <= Player:GCD() or not bool(S.Deathbolt:CooldownRemainsP()) or active_enemies() > 1)) then
    ...
    if S.UnstableAffliction:IsReadyP() and (not Player:IsMoving()) and not Player:ShouldStopCasting() and not bool(contagion) and Target:TimeToDie() <= 8 then
      return S.UnstableAffliction:Cast()
    end
    ...
    if S.DrainSoul:IsCastableP() and Target:DebuffRemainsP(S.ShadowEmbraceDebuff) and S.ShadowEmbrace:IsAvailable() and bool(VarMaintainSe) and Target:DebuffRemainsP(S.ShadowEmbraceDebuff) and Target:DebuffRemainsP(S.ShadowEmbraceDebuff) <= Player:GCD() * 2 then
      return S.DrainSoul:Cast()
    end
    ...
    if S.ShadowBolt:IsCastableP() and (not Player:IsMoving()) and not Player:ShouldStopCasting() and S.ShadowEmbrace:IsAvailable() and bool(VarMaintainSe) and Target:DebuffRemainsP(S.ShadowEmbraceDebuff) and Target:DebuffRemainsP(S.ShadowEmbraceDebuff) <= S.ShadowBolt:ExecuteTime() * 2 + 1 and (not S.ShadowBolt:InFlight()) then
      return S.ShadowBolt:Cast()
    end
    ...
    if S.PhantomSingularity:IsCastableP() and HL.CombatTime() > 35 and (S.SummonDarkglare:CooldownRemainsP() >= 45 or S.SummonDarkglare:CooldownRemainsP() < 8) and Target:TimeToDie() > 16 * Player:SpellHaste() then
    ...
    if S.SeedofCorruption:IsCastableP() and RubimRH.AoEON() and active_enemies() > 3 and (not Player:IsMoving()) and not Player:ShouldStopCasting() and (bool(VarUseSeed) and Player:SoulShardsP() == 5) then
      return S.SeedofCorruption:Cast()
    end
    -- call_action_list,name=dots
    if (true) then
		local ShouldReturn = Dots(); 
			if ShouldReturn then return ShouldReturn; 
		end
    end
