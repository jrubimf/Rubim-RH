...
(
    ...
    if S.DarkSoul:IsCastableP() and RubimRH.CDsON() and (S.SummonDarkglare:CooldownRemainsP() < 10 and (Target:DebuffP(S.PhantomSingularityDebuff) or Target:DebuffP(S.PhantomSingularityDebuff) or not S.PhantomSingularity:IsAvailable() and not S.VileTaint:IsAvailable()) or Target:TimeToDie() < 20 + Player:GCD() or EnemiesCount > 1) then
