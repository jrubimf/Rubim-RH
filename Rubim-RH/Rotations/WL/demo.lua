...
RubimRH.Spell[266] = {
  ...
  VisionOfPerfection2          = Spell(299368),
  VisionOfPerfection3          = Spell(299370),
 -- Lifeblood                    = MultiSpell(295137, 305694),
  Lifeblood                    = Spell(295137),
  Lifeblood2                   = Spell(305694),
};
...
local function DetermineEssenceRanks()
	...
	--S.Lifeblood = S.Lifeblood2:IsAvailable() and S.Lifeblood2 or S.Lifeblood
end
...
local function APL()
  ...
  Precombat_DBM = function()
    ...
    if I.PotionofUnbridledFury:IsReady() and RubimRH.DBM_PullTimer() >= S.Demonbolt:CastTime() + 1 and RubimRH.DBM_PullTimer() <= S.Demonbolt:CastTime() + 2 then
        return 967532
    end
  ...
  BuildAShard = function()
    ...
    if S.MemoryOfLucidDreams:IsCastableP() and (Player:SoulShardsP() < 2) and RubimRH.CDsON() then
  ...
  if RubimRH.TargetIsValid() then
    ...
    if I.PotionofUnbridledFury:IsReady() and (DemonicTyrantTime() > 0 and (not S.VisionOfPerfection:IsAvailable() or not S.DemonicConsumption:IsAvailable() or S.SummonDemonicTyrant:CooldownRemainsP() >= S.SummonDemonicTyrant:BaseDuration() - 5) and (not S.NetherPortal:IsAvailable() or S.NetherPortal:CooldownRemainsP() > 160)) then
      return 967532
    end
    ...
    if S.Berserking:IsCastableP() and RubimRH.CDsON() and (DemonicTyrantTime() > 0 and (not S.VisionOfPerfection:IsAvailable() or not S.DemonicConsumption:IsAvailable() or S.SummonDemonicTyrant:CooldownRemainsP() >= S.SummonDemonicTyrant:BaseDuration() - 5)) then
    ...
    if S.BloodFury:IsCastableP() and RubimRH.CDsON() and (DemonicTyrantTime() > 0 and (not S.VisionOfPerfection:IsAvailable() or not S.DemonicConsumption:IsAvailable() or S.SummonDemonicTyrant:CooldownRemainsP() >= S.SummonDemonicTyrant:BaseDuration() - 5)) then
    ...
    if S.Fireblood:IsCastableP() and RubimRH.CDsON() and (DemonicTyrantTime() > 0 and (not S.VisionOfPerfection:IsAvailable() or not S.DemonicConsumption:IsAvailable() or S.SummonDemonicTyrant:CooldownRemainsP() >= S.SummonDemonicTyrant:BaseDuration() - 5)) then
      return S.Fireblood:Cast()
    end
    -- trinket1,if=pet.demonic_tyrant.active
    if trinketReady(1) and RealTyranIsActive() and RubimRH.CDsON() then
        return trinket1
    end
    -- trinket2,if=pet.demonic_tyrant.active
   	if trinketReady(2) and RealTyranIsActive() and RubimRH.CDsON() then
        return trinket2
    end
    ...
    if S.WorldveinResonance:IsCastableP() and (Player:BuffStackP(S.Lifeblood) < 3 and (DemonicTyrantTime() > 0 and (not S.VisionOfPerfection:IsAvailable() or not S.DemonicConsumption:IsAvailable() or S.SummonDemonicTyrant:CooldownRemainsP() >= S.SummonDemonicTyrant:BaseDuration() - 5))) then
      return S.UnleashHeartOfAzeroth:Cast()
    end
    ...
    if (S.NetherPortal:IsAvailable() and HL.CombatTime() < 30 and not bool(S.SummonDemonicTyrant:CooldownRemainsP())) then
