...
local function APL()
    ...
    FiveTarget = function()
        ...
        if S.Whirlwind:IsReady() and (Target:DebuffP(S.ColossusSmashDebuff)) then
    ...
    if S.AncestralCall:IsReady() and (Target:DebuffP(S.ColossusSmashDebuff)) then
...
local function five_target()
    ...
    if S.Bladestorm:IsReady() and Player:Buff(S.BattleCry) and not S.Ravager:IsAvailable() and Cache.EnemiesCount[8] >= 1 then
        return S.Bladestorm:Cast()
    end
