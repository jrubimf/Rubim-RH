...
local function APL()
  ...
  Precombat = function()
    ...
    if S.Metamorphosis:IsCastableP(40) and RubimRH.CDsON() and (Player:BuffDownP(S.MetamorphosisBuff) and not S.ChaoticTransformation:AzeriteEnabled()) then
      return S.Metamorphosis:Cast()
    end
  end
  ...
  Cooldown = function()
    ...
    if S.Berserking:IsCastableP() and RubimRH.CDsON() and Player:BuffP(S.MetamorphosisBuff) then
      return S.Berserking:Cast()
    end
    -- blood_fury,if=buff.metamorphosis.up
    if S.BloodFury:IsCastableP() and RubimRH.CDsON() and Player:BuffP(S.MetamorphosisBuff) then
      return S.BloodFury:Cast()
    end
    -- fireblood,if=buff.metamorphosis.up
    if S.Fireblood:IsCastableP() and RubimRH.CDsON() and Player:BuffP(S.MetamorphosisBuff) then
  ...
  Demonic = function()
    ...
    if S.FelRush:IsCastableP(20, true) and (S.DemonBlades:IsAvailable() and not S.EyeBeam:CooldownUpP()) then
  ...
  Normal = function()
    ...
    if S.FelRush:IsCastableP(20, true) and ((bool(VarWaitingForMomentum) or S.FelMastery:IsAvailable())) then
        return S.FelRush:Cast()
    end
    ...
    if S.FelRush:IsCastableP(20, true) and (not S.Momentum:IsAvailable() and S.DemonBlades:IsAvailable()) then
      return S.FelRush:Cast()
    end
