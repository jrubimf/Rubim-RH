...
local function Stealthed ()
    ...
    local VanishBuffCheck = Player:Buff(VanishBuff)
    ...
    if S.Shadowstrike:IsCastable() and (Target:IsInRange(S.Shadowstrike) or IsInMeleeRange()) and (S.FindWeakness:IsAvailable() or Cache.EnemiesCount[10] < 3) and (StealthBuff or VanishBuffCheck) then
        if ReturnSpellOnly then
            return S.Shadowstrike:Cast()
        else
            S.Shadowstrike:Queue()
        end
    end
...
local function CDs ()
    ...
    if RubimRH.CDsON() then
        ...
        if S.ShadowBlades:IsReady() and not Player:Buff(S.ShadowBlades) and Player:ComboPointsDeficit() >= 2 + num(Player:IsStealthedP(true, true)) then
