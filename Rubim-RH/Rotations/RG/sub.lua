...
local function Build ()
	...
	if RubimRH.AoEON() and S.ShurikenStorm:IsReady() and Cache.EnemiesCount[10] >= 2 then
		return S.ShurikenStorm:Cast()
	end
...
local function APL()
	...
	if RubimRH.TargetIsValid() then
		...
		if S.ArcanePulse:IsReady() and IsInMeleeRange() then
		...
		local StealthBuff = Player:Buff(Stealth) or (StealthSpell and StealthSpell:ID() == Stealth:ID())
		if StealthBuff and S.Shadowstrike:IsReady() and IsInMeleeRange() then
			return S.Shadowstrike:Cast()
		end
	end
	return 0, 135328
