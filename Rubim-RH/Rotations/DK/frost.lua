...
local function BoSPool()
    ...
    if S.Obliterate:IsReady() and Player:RuneTimeToX(6) < Player:GCD() and not S.GatheringStorm:IsAvailable() then
        return S.Obliterate:Cast()
    end
    --actions.bos_pooling+=/obliterate,if=rune.time_to_4<gcd&(cooldown.breath_of_sindragosa.remains|runic_power.deficit>=30)
    if S.Obliterate:IsReady() and Player:RuneTimeToX(4) < Player:GCD() and (S.BreathofSindragosa:CooldownRemainsP() or Player:RunicPowerDeficit() >= 30) then
        return S.Obliterate:Cast()
    end
    ...
    if S.Obliterate:IsReady() and not Player:Buff(S.Rime) and not (S.GatheringStorm:IsAvailable() and not (S.RemorselessWinter:CooldownRemainsP() > (Player:GCD() * 2) or Player:Runes() > 4)) and Player:Runes() > 3 then
        return S.Obliterate:Cast()
    end
    ...
    if S.Obliterate:IsReady() and not Player:Buff(S.Rime) and (not S.GatheringStorm:IsAvailable() or S.RemorselessWinter:CooldownRemainsP() > Player:GCD()) then
        return S.Obliterate:Cast()
    end
...
local function BoSTick()
    ...
    if S.Obliterate:IsReady() and (Player:RunicPower() <= 45 or Player:RuneTimeToX(5) < Player:GCD() or Player:BuffRemainsP(S.EmpowerRuneWeapon) >= 2) then
        return S.Obliterate:Cast()
    end
...
local function Standard()
    ...
    if S.Obliterate:IsReady("Melee") and ((I.KoltirasNewfoundWill:IsEquipped() and S.FrozenPulse:IsAvailable() and T192)) then
        return S.Obliterate:Cast()
    end
    ...
    if S.Obliterate:IsReady("Melee") and ((I.KoltirasNewfoundWill:IsEquipped() and S.FrozenPulse:IsAvailable() and T192) or Player:RuneTimeToX(5) < Player:GCD()) then
        return S.Obliterate:Cast()
    end
    ...
    if S.Obliterate:IsReady("Melee") and Player:Buff(S.KillingMachine) then
        return S.Obliterate:Cast()
    end
